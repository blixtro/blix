...
async function getAIReply(userMessage, products) {
    ...
    for (const model of models) {
        try {
            ...
            console.log(`✅ Groq (${model}) رد:`, reply.substring(0, 80) + '...');
            return reply;
        } catch (err) {
            console.warn(`⚠️ ${model} فشل، جاري المحاولة بالموديل التاني...`);
        }
    }
...
app.get('/test-db', async (_req, res) => {
    const { data, error } = await supabase.from('blixtro').select('*');
    res.json({
        count: data?.length || 0,
...
// ─── GET /ping?msg=رسالتك  (بدون Supabase) ───────────────────────────────────
app.get('/ping', async (req, res) => {
    const msg = req.query.msg || 'مرحبا';
    try {
        const result = await groq.chat.completions.create({
            model: 'llama-3.1-8b-instant',
            messages: [
                { role: 'system', content: 'أنت مساعد ودود. رد بشكل طبيعي وقصير.' },
                { role: 'user', content: msg }
            ],
            max_tokens: 200,
        });
        const reply = result.choices[0]?.message?.content || '';
        res.json({ message: msg, reply });
    } catch (err) {
...
app.get('/chat', async (req, res) => {
    ...
    if (!msg) return res.json({ error: 'أكتب رسالتك في الرابط: /chat?msg=رسالتك' });

    const products = await fetchProducts();
    const reply    = await getAIReply(msg, products);
